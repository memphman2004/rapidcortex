#!/usr/bin/env bash
set -euo pipefail
# One-command deploy: build workspaces and deploy SAM backend stack 2 (infra/template2.yaml) with optional domain parameters.
# Note: infra/template.yaml (primary root stack) also nests stack-app-sam-2 as AppSam2Stack — use this script only when you need a **separate** CloudFormation stack for stack 2 (e.g. legacy split deployments).
# Usage: ./scripts/deploy2.sh [dev|staging|prod|pilot] [--changeset-only|--no-execute-changeset]
...
# - STACK_NAME (optional) → CloudFormation stack name; default ${APP_NAME}-2-${STAGE}
...
# - INCLUDE_DATA_LAYER_NESTED_STACK=false … (requires FLAT_DATA_LAYER_* ARNs — see deploy.sh comments).
# - IMPORTED_COGNITO_USER_POOL_ID, IMPORTED_COGNITO_WEB_CLIENT_ID, IMPORTED_COGNITO_ISSUER (required) — copy from stack 1 outputs
#   (e.g. rapid-cortex-dev → UserPoolId, UserPoolClientId, CognitoIssuer). Optional:
#   IMPORTED_COGNITO_NATIVE_CLIENT_ID (NativeUserPoolClientId), IMPORTED_COGNITO_HOSTED_UI_BASE (CognitoHostedUiBase).
# - IMPORTED_OPS_ALERTS_TOPIC_ARN (optional) — stack 1 `OpsAlertsTopicArn` output; when set, contact-sales SNS + other
#   flows use that topic instead of creating a second ops topic in stack 2 only for those overrides (see SAM template).
# - SKIP_CFN_DIAG_ON_DEPLOY_FAIL=1 skip AWS CLI diagnostics after a failed sam deploy (default: print them).
...
# - SAM_BUILD_DIR explicit sam build output directory (mkdir -p before build; roomy disk recommended).
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: ./scripts/deploy2.sh [dev|staging|prod|pilot] [--changeset-only|--no-execute-changeset]" >&2
      exit 1
      ;;
  esac
done
...
if [[ -z "${IMPORTED_COGNITO_USER_POOL_ID:-}" || -z "${IMPORTED_COGNITO_WEB_CLIENT_ID:-}" || -z "${IMPORTED_COGNITO_ISSUER:-}" ]]; then
  echo "ERROR: ./scripts/deploy2.sh requires Cognito values from the primary stack (template.yaml), e.g. rapid-cortex-dev outputs:" >&2
  echo "  export IMPORTED_COGNITO_USER_POOL_ID=\"\$(aws cloudformation describe-stacks --stack-name rapid-cortex-dev --query 'Stacks[0].Outputs[?OutputKey==\`UserPoolId\`].OutputValue' --output text)\"" >&2
  echo "  export IMPORTED_COGNITO_WEB_CLIENT_ID=\"\$(aws cloudformation describe-stacks --stack-name rapid-cortex-dev --query 'Stacks[0].Outputs[?OutputKey==\`UserPoolClientId\`].OutputValue' --output text)\"" >&2
  echo "  export IMPORTED_COGNITO_ISSUER=\"\$(aws cloudformation describe-stacks --stack-name rapid-cortex-dev --query 'Stacks[0].Outputs[?OutputKey==\`CognitoIssuer\`].OutputValue' --output text)\"" >&2
  echo "Optional: IMPORTED_COGNITO_NATIVE_CLIENT_ID, IMPORTED_COGNITO_HOSTED_UI_BASE from NativeUserPoolClientId / CognitoHostedUiBase." >&2
  exit 1
fi
...
sam validate --template-file "${ROOT}/infra/template2.yaml"
...
# SAM CLI may rmtree SAM_BUILD_DIR — use roomy path (SAM_BUILD_DIR) or disposable mktemp.
if [[ -n "${SAM_BUILD_DIR:-}" ]]; then
  mkdir -p "${SAM_BUILD_DIR}"
else
  SAM_BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/rapid-cortex-sam-build.XXXXXX")"
fi
echo "SAM build directory: ${SAM_BUILD_DIR}"
sam_build_failed=0
SAM_BUILD_CLI=(sam build --template-file infra/template2.yaml --build-dir "${SAM_BUILD_DIR}")
...
RC_SAM_BUILD_DIR="${SAM_BUILD_DIR}" RC_ROOT_TEMPLATE_FILE=infra/template2.yaml bash "${ROOT}/scripts/infra-template-size-check.sh"
...
PARAMS="${PARAMS} ImportedCognitoUserPoolId=${IMPORTED_COGNITO_USER_POOL_ID}"
PARAMS="${PARAMS} ImportedCognitoWebClientId=${IMPORTED_COGNITO_WEB_CLIENT_ID}"
PARAMS="${PARAMS} ImportedCognitoIssuer=${IMPORTED_COGNITO_ISSUER}"
if [[ -n "${IMPORTED_COGNITO_NATIVE_CLIENT_ID:-}" ]]; then
  PARAMS="${PARAMS} ImportedCognitoNativeClientId=${IMPORTED_COGNITO_NATIVE_CLIENT_ID}"
fi
if [[ -n "${IMPORTED_COGNITO_HOSTED_UI_BASE:-}" ]]; then
  PARAMS="${PARAMS} ImportedCognitoHostedUiBase=${IMPORTED_COGNITO_HOSTED_UI_BASE}"
fi
if [[ -n "${IMPORTED_OPS_ALERTS_TOPIC_ARN:-}" ]]; then
  PARAMS="${PARAMS} ImportedOpsAlertsTopicArn=${IMPORTED_OPS_ALERTS_TOPIC_ARN}"
fi
...
rapid_cortex_print_deploy_failure_reason() {
  echo "" >&2
  echo "════════════════════════════════════════════════════════════════" >&2
  echo " SAM deploy failed — CloudFormation diagnostics (deploy2.sh)" >&2
  ...
  echo "" >&2
  echo "Tip: full event history: aws cloudformation describe-stack-events --stack-name '${STACK_NAME}'" >&2
}
