...
const config = applyKcpd
  ...
  : {
      ...
      tenantCity: process.env.AGENCY_CITY?.trim() || undefined,
      callAssistGreeting: defaultGreetingConfig({
        cityName: process.env.AGENCY_CITY?.trim() || "",
        agencyName,
        previewConfirmed: Boolean(process.env.AGENCY_CITY?.trim() && agencyName),
        emergencyTransferNumber: process.env.EMERGENCY_LINE?.trim() || "911",
      }),
      retention: {
        ...MISSOURI_SUNSHINE_RETENTION_POLICY,
