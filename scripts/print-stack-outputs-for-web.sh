...
WEBSOCKET_API_URL="$(get_output WebSocketApiUrl)"
...
if [[ -n "${GRANT_GENERATE_FUNCTION_NAME:-}" && "$GRANT_GENERATE_FUNCTION_NAME" != "None" ]]; then
  echo ""
  echo "# --- Grant PDF (BFF→Lambda Invoke; bypasses API Gateway 30s cap) ---"
...
echo ""
echo "# --- Guest Assist (Claude via Next BFF; staff chat via WebSocket HMAC) ---"
if [[ -n "${WEBSOCKET_API_URL}" && "$WEBSOCKET_API_URL" != "None" ]]; then
  echo "NEXT_PUBLIC_WEBSOCKET_URL=${WEBSOCKET_API_URL}"
else
  echo "# NEXT_PUBLIC_WEBSOCKET_URL=(WebSocketApiUrl not in stack outputs)"
fi
echo "NEXT_PUBLIC_ENABLE_GUEST_ASSIST=1"
echo "# GUEST_ASSIST_SESSION_SECRET=(same value on web ECS and AppSamRealtime2Stack)"
echo "# ANTHROPIC_API_KEY is injected on ECS from ANTHROPIC_API_KEY_SECRET_ARN — never put it in the static HTML"
echo ""
echo "# --- Occupant alerts / ENS / Four Winds (default on when unset) ---"
echo "NEXT_PUBLIC_ENABLE_VERTICAL_ALERTS=1"
echo "NEXT_PUBLIC_ENABLE_ENS_TEST_PROGRAM=1"
echo "NEXT_PUBLIC_ENABLE_FOURWINDS=1"
FW_ARN="$(get_output FourWindsSecretArn 2>/dev/null || true)"
if [[ -n "${FW_ARN}" && "$FW_ARN" != "None" ]]; then
  echo "# API EXISTING_FOURWINDS_SECRET_ARN=${FW_ARN}"
fi
echo "# Live display takeover: bash scripts/put-fourwinds-secret.sh --api-key '…' && FOURWINDS_MOCK=false"
echo ""
