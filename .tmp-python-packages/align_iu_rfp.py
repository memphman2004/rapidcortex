#!/usr/bin/env python3
"""Align IU Component C SOW + narrative with NexCort iQ product facts."""
...
EXCEL_UPDATES = {
    "SOC-001": (
        PARTIAL,
        "NexCort iQ is an API-first, vendor-agnostic situational-awareness layer. REST/JSON APIs, webhooks, and normalized incident/event ingest are in production today. Device-layer protocols (ONVIF, OSDP, SIA, MQTT, Wiegand) remain with IU’s source systems (Milestone XProtect, CBORD/CS Gold, sensors) or are delivered as Year-1 connectors; NexCort iQ does not terminate those protocols itself. IU keeps best-of-breed subsystems; NexCort iQ unifies incidents, maps, audit, and operator workflows without a single-vendor lock-in.",
    ),
    "SOC-007": (
        PARTIAL,
        "NexCort iQ natively associates cameras, QR/NFC locations, buildings, floors, and zones to geospatial records and uses that mapping for nearest-resource and incident context. Mapping emergency phones (Component B) or access points to cameras is IU configuration plus the relevant connector. The geographic association model is live; IU-specific phone-to-camera tables will be loaded during implementation.",
    ),
    "SOC-022": (
        PARTIAL,
        "The NexCort iQ event model consolidates alerts from multiple sources into one queue, map, and dashboard with source attribution. Production ALPR connectors (Flock, Genetec AutoVu, or other IU-named platforms) are implementation work, not pre-certified adapters. Once connected, operators see a unified view; the ALPR vendor remains the system of record for plate recognition and hot-list administration.",
    ),
    "SOC-027": (
        PARTIAL,
        "NexCort iQ provides a live, map-based campus and venue command picture (Mapbox) with incidents, QR/NFC/SMS intake, campus-scoped RBAC, and a nine-campus enterprise view for authorized staff. Responder GPS, EMNS delivery receipts, and live VMS wall status appear when those source systems provide them through Year-1 connectors. NexCort iQ does not replace IU’s EMNS, officer AVL, or Milestone as systems of record.",
    ),
    "SOC-028": (
        PARTIAL,
    ...
    "UM-019": (
        PARTIAL,
        "NexCort iQ uses a canonical RBAC model with predefined campus, venue, hospital, transit, and PSAP roles—including campus_admin, campus_supervisor, campus_security, campus_counselor, and campus_faculty—scoped by agencyId and campus. IU cannot invent arbitrary custom permission sets in a product role-builder; additional roles are contracted configuration, not an in-product policy editor.",
    ),
    "INT-026": (
        PARTIAL,
        "Native operational mapping is Mapbox. Campus/venue zones, incident pins, and overlays are in production. Esri ArcGIS or other GIS layers can be ingested as web services/overlays during implementation. NexCort iQ is not an ArcGIS replacement.",
    ),
    "ITS-012": (
        PARTIAL,
        "NexCort iQ is cloud SaaS. Priority 1 production incidents are acknowledged within 30 minutes via on-call escalation, with remote diagnosis as the primary remediation path. NexCort iQ does not staff a 24/7 security operations center on IU’s behalf and does not dispatch field technicians or spare parts for IU hardware. The four-hour on-site hardware response in this requirement does not apply to Component C; device repair remains with EMNS, blue-light, VMS, and related vendors. Awareness-only sites receive the same remote software support.",
    ),
    "INT-027": (
        PARTIAL,
        "In-platform war rooms (collaborative incident command) are a NexCort iQ product surface. Deep Microsoft Teams meeting/channel integration is an optional Year-1 module ($8,000/year) via Microsoft Graph APIs, subject to IU Teams telephony permissions. Slack can use equivalent webhook/API patterns.",
    ),
    "SOC-002": (
        FULLY,
        "Subsystem integrations are decoupled from the NexCort iQ incident and operational data model. A camera, ALPR, access-control, or sensor source can be added or replaced by changing its connector while preserving incidents, workflows, dashboards, audit history, and campus configuration. Live today this is proven with Ring, AWS KVS, and RTSP/ONVIF-capable camera paths; Milestone/CBORD/ALPR follow the same connector pattern in Year 1.",
    ),
    "SOC-026": (
        FULLY,
        "NexCort iQ provides native incident creation, assignment, status changes, notes, media, escalation, and closure with a unified chronological continuity log. Intake channels include QR, NFC, dedicated SMS, console entry, and webhooks. In-platform war rooms support major-incident coordination. Events from integrated systems attach to the same incident so operators keep a single operational record.",
    ),
    "TI-004": (
        FULLY,
        "NexCort iQ uses logical tenant isolation: every DynamoDB access is scoped by agencyId, and JWT role/campus claims enforce RBAC. IU campuses/buildings/zones can be segmented so local operators see only authorized scope while enterprise Public Safety leadership can receive a consolidated nine-campus view. Tenants share AWS services (Lambda, tables, S3) with partition-level isolation; other customers cannot read IU records. URL slugs are not the security boundary.",
    ),
}

# Additional Excel explanation-only updates (keep rating)
EXCEL_EXPLAIN_ONLY = {
    "SOC-012": "NexCort iQ is camera-vendor agnostic and consumes camera/VMS context rather than replacing the VMS. Hanwha Wisenet discovery, PTZ, and H.265 remain with Milestone XProtect / ONVIF-capable infrastructure. Direct RTSP/KVS paths exist for venue/campus live video; IU’s preferred architecture is Hanwha via XProtect, which is a Year-1 connector workstream.",
    "SOC-015": "Privacy masking of live/recorded imagery remains controlled by Milestone XProtect / Hanwha so the authoritative privacy configuration is preserved. NexCort iQ enforces role-based access, campus/zone scoping, and audit around incident-linked video. It does not apply pixel masks inside a NexCort iQ camera UI.",
    "SOC-032": "ARMS by End2End is a priority Year-1 integration. NexCort iQ can ingest CAD incident type/location and exchange status through available ARMS interfaces. CAD write-back, if IU elects it, is fail-closed and requires human review; NexCort iQ does not auto-create CAD records. Exact bi-directional functions depend on End2End interface availability and licensing.",
    "INT-025": "ARMS by End2End is a priority integration. NexCort iQ can ingest CAD incident type/location and exchange incident/status data through available ARMS interfaces. CAD write-back is fail-closed and human-approved. Other CAD platforms can be integrated through documented APIs; NexCort iQ does not replace ARMS as the CAD/RMS of record.",
    ...
    "ITS-028": "NexCort iQ will provide 24/7/365 on-call escalation for Priority 1 production software incidents with phone/on-call access, plus email/portal support for lower priorities. This is software on-call, not a staffed NexCort iQ SOC operating IU’s cameras and doors. Final support channels and escalation contacts will be documented in the SLA.",
    "STD-011": "NexCort iQ will document applicable security-framework alignment and AWS inherited controls in the HECVAT/security package. AWS services carry their own SOC 2 / ISO / FedRAMP attestations; those are not NexCort iQ product certifications. NexCort iQ does not claim ISO 27001, SAFETY Act, or SOC 2 Type II for the NexCort iQ application unless separately evidenced at submission.",
}
...
PARA = {
    "NexCort iQ fully understands Indiana University's phased implementation approach and has designed its proposal to deliver standalone value for Component C while maintaining clear integration paths to the full platform vision as described in Section F of the RFP.": (
        "NexCort iQ is proposing the campus-native operational layer IU can run on day one while coexistence with incumbent systems continues. "
        ...
        "NexCort iQ does not replace 911, CAD, Milestone, CBORD, Clery statutory filing, or IU’s chosen EMNS and blue-light vendors. "
        "Ratings in the attached SOW workbook are controlling; this narrative expands those ratings and is aligned to them."
    ),
    "The NexCort iQ platform delivers real-time incident intelligence, multi-channel safety reporting via QR codes, NFC tags, and dedicated SMS lines, live transcription, bidirectional multilingual translation across 40-plus languages, AI-powered incident classification, supervisor command visibility, and integrated camera access — from a single unified cloud-native platform. The system is designed to operate alongside existing infrastructure without requiring replacement of any incumbent technology.": (
        "The NexCort iQ platform delivers real-time incident intelligence; multi-channel safety reporting via QR codes, NFC tags, and dedicated SMS lines; "
    ...
    "The NexCort iQ platform is designed with Clery Act compliance requirements as a core architectural consideration. Relevant capabilities include:": (
        "NexCort iQ supports Clery-adjacent operational recordkeeping. Clery Act statutory filings, Annual Security Reports, and Emergency Notification / Timely Warning issuance remain IU’s responsibility and, for mass notification, the Component A EMNS vendor. Relevant NexCort iQ capabilities include:"
    ),
    "Incident classification workflows support Emergency Notification and Timely Warning distinctions with separate templates, approval workflows, and audit trails.": (
        "Incident classification can record Clery-relevant categories and geography to support IU’s Clery workflow. NexCort iQ does not send Emergency Notifications or Timely Warnings; those remain IU policy plus the selected EMNS."
    ),
    "System testing documentation including date, time, scope, and delivery results suitable for Clery Act test documentation requirements.": (
        "Timestamped exercise and incident/audit records suitable for after-action documentation. Audible/channel delivery tests remain with the EMNS and blue-light systems; NexCort iQ can log integrated test events when those systems notify it."
    ),
    "NexCort iQ is designed with student data privacy as a first-order requirement. The platform does not store student education records and does not expose personally identifiable student information in operational interfaces without explicit role authorization. Specifically:": (
        "NexCort iQ is designed with student data privacy as a first-order requirement. Safety reports submitted via QR, NFC, or SMS may contain personally identifiable information; those records are operational public-safety records, not a student-information system. FERPA applicability is an IU legal/policy determination under a Data Processing Agreement. NexCort iQ will not use IU data for marketing. Specifically:"
    ),
    ...
    "Privacy masking zones can be configured for camera streams near healthcare, counseling, or sensitive facilities.": (
        "Camera privacy masking for healthcare, counseling, or other sensitive facilities remains in Milestone XProtect / Hanwha. NexCort iQ restricts who can open incident-linked video by role, campus, and audit."
    ),
    ...
    "The NexCort iQ anonymous tip reporting system supports confidential incident submission without requiring reporter identification. This enables students to report Title IX concerns through QR codes, NFC tags, or dedicated SMS lines without risk of identity exposure. The back-end workflow supports tip intake, assignment to appropriate staff, and anonymous follow-up with the reporter while preserving anonymity throughout the chain.": (
        "Anonymous QR/NFC/SMS intake supports confidential reporting without requiring reporter identification, including concerns that IU may later handle under Title IX. Reports can be assigned to campus counselor or security roles. NexCort iQ is not a Title IX case-management system, investigation file, or hearing platform; IU’s Title IX office remains system of record for those processes."
    ),
    "NexCort iQ delivers SMS through AWS End User Messaging as its primary carrier infrastructure — a direct AWS service with established relationships with all major US wireless carriers including AT&T, T-Mobile, Verizon, and regional carriers. This is not an aggregator model. AWS End User Messaging delivers messages through direct carrier connections, providing enterprise-grade reliability, throughput, and delivery transparency.": (
        "NexCort iQ delivers two-way SMS through AWS End User Messaging as its primary path, with Twilio as a secondary provisioning path for campus/venue dedicated lines. AWS End User Messaging is an AWS-managed messaging service with US carrier reach. NexCort iQ uses it for conversational incident reporting (Component C), not as IU’s mass-notification engine."
    ),
    "Carrier-level delivery receipts — per-message delivery status tracking to individual handsets": (
        "Delivery status as provided by AWS End User Messaging / Twilio (message accepted/failed at the provider). Handset-level carrier receipts are not independently guaranteed by NexCort iQ."
    ),
    ...
    "Penetration testing: Annual third-party penetration testing of the platform.": (
        "Penetration testing: NexCort iQ will contract annual third-party penetration testing of the platform under the IU agreement and share results under NDA."
    ),
    "NexCort iQ acknowledges and commits to compliance with all data types specified in Question 20(b), including FERPA, HIPAA (with BAA if PHI is processed), GLBA, Indiana SSN Law, Indiana Breach Notification Laws, and PCI-DSS. PCI-DSS is not applicable to the core RC Campus platform as the platform does not process payment card data. If any future module requires payment processing, a third-party PCI-compliant payment processor will be used.": (
        "NexCort iQ will operate under an IU Data Processing Agreement and align technical and organizational controls to IU’s requirements for FERPA, applicable Indiana privacy and breach-notification laws, and GLBA where IU determines those laws apply to the data in this system. HIPAA: a BAA in IU-approved form will be executed before any PHI is processed; NexCort iQ does not claim HIPAA certification. PCI-DSS is not applicable to Component C (no cardholder data). NexCort iQ does not claim FERPA, HIPAA, CJIS, or SOC 2 certification in this proposal."
    ),
    ...
    "MQTT support for IoT sensor data ingestion": (
        "IoT/sensor ingest via REST, webhooks, or MQTT gateways provided by the source system or a Year-1 connector — NexCort iQ does not require IU to re-wire sensors"
    ),
    ...
    "SOC-012 — Hanwha Wisenet: RC's ONVIF-compliant camera integration supports camera discovery, streaming (RTSP), and PTZ control for any ONVIF Profile S, G, or T camera — including the Hanwha Wisenet product line. Hanwha cameras can be integrated directly into the RC camera management layer without the Milestone XProtect intermediary for use cases where direct ONVIF streaming is preferred. Hanwha's Wisenet WAVE and Wisenet SSM platforms also support open API access that RC can leverage for deeper integration.": (
        "SOC-012 — Hanwha Wisenet: NexCort iQ is not a camera VMS. IU’s preferred path is Hanwha cameras managed in Milestone XProtect, with live/recorded context presented in the NexCort iQ SOC console through the Year-1 XProtect connector. Where IU provides an RTSP/ONVIF URL, NexCort iQ can stream via AWS KVS/WebRTC for incident-linked live video. PTZ, Profile S/G/T discovery, and H.265 multi-stream remain XProtect/Hanwha functions."
    ),
    "SOC-013: RC supports H.264 and H.265 video streams via WebRTC and HLS delivery paths. Multi-stream configurations (high-res recording + low-res live preview) are supported. Edge recording is not a native RC function — RC streams live video; recording is handled by the VMS (Milestone XProtect).": (
        "SOC-013: NexCort iQ can present H.264/H.265 streams made available by the VMS or a direct RTSP/KVS path via WebRTC. Multi-stream and edge recording remain Milestone/Hanwha functions. NexCort iQ does not transcode IU’s camera fleet or replace XProtect recording."
    ),
    "SOC-015: Privacy masking zones are configurable per camera stream in the RC camera management interface. Masking is applied at the display layer within the RC console — cameras near healthcare, counseling, or FERPA-sensitive facilities can be restricted by role. HIPAA-sensitive camera streams can be limited to users with explicit permission grants.": (
        "SOC-015: Privacy masking of camera imagery is configured in Milestone XProtect / Hanwha so IU’s authoritative privacy zones are preserved. NexCort iQ restricts who may open incident-linked video by role and campus, and audits that access. NexCort iQ does not apply pixel masks in a NexCort iQ camera-management UI."
    ),
    "SOC-017: RC supports vendor-agnostic access control integration via REST API (preferred), OSDP, and legacy Wiegand where required for backward compatibility. Supported or planned platforms include CBORD/CS Gold (Year 1), Lenel S2, Genetec Synergis, Honeywell Pro-Watch, and Software House C-CURE. All access events are normalized into RC's standard event schema before display.": (
        "SOC-017: NexCort iQ integrates access control through REST/webhooks/middleware. OSDP and Wiegand remain device/controller-layer protocols terminated by CBORD (or another ACS), not by NexCort iQ. CBORD/CS Gold is the Year-1 IU connector. Additional ACS vendors can follow the same event schema; they are not represented as currently certified connectors."
    ),
    ...
    "Push RC incident data to ARMS for CAD record creation": (
        "Share NexCort iQ incident context with ARMS under IU-approved, human-reviewed workflows — CAD write-back is fail-closed and is not automatic"
    ),
    ...
    "SOC-042 — AI/ML Features (RapidIQ): NexCort iQ's RapidIQ AI engine is the most significant capability differentiator in this proposal. RapidIQ is a proprietary multi-model AI inference pipeline built on Anthropic Claude (via AWS Bedrock) as the primary model, with OpenAI GPT as secondary fallback and direct Anthropic API as tertiary. RapidIQ provides:": (
        "SOC-042 — AI/ML Features: NexCort iQ incident intelligence is a multi-model pipeline on AWS Bedrock (Anthropic Claude primary, with configured OpenAI and other fallbacks). This is operator decision support — not an autonomous dispatch engine. Capabilities include:"
    ),
    ...
    "INT-014 — PSIM/VMS: RC serves as the PSIM layer for the RC Campus deployment. VMS integration via Milestone XProtect is committed for Year 1 as described in SOC-003 through SOC-010.": (
        "INT-014 — PSIM/VMS: NexCort iQ is the unifying situational-awareness and incident layer for Component C. It is not a traditional PSIM appliance and does not replace Milestone XProtect. XProtect integration is a Year-1 Founding Partner commitment as described in SOC-003 through SOC-010."
    ),
    ...
    "UM-019 — RBAC: RC provides nine pre-defined roles with granular, configurable permissions. For Indiana University, the relevant roles include:": (
        "UM-019 — RBAC: NexCort iQ uses predefined canonical roles (not an arbitrary permission-set builder). Campus-relevant roles include:"
    ),
    "Custom roles with granular permission sets can be configured by RC for IU-specific requirements.": (
        "Additional roles or permission changes beyond the canonical set are contracted configuration performed by NexCort iQ with IU, not a self-serve custom-role editor in the product."
    ),
    ...
    "INT-026 — GIS: RC uses Mapbox for geographic incident visualization. ArcGIS integration is available via the ArcGIS REST API for GIS-based targeting and campus map overlays. Google Maps API is also supported.": (
        "INT-026 — GIS: Native operational mapping is Mapbox (campus/venue zones and incident visualization). Esri ArcGIS or other GIS layers can be ingested as overlays/web services during implementation. NexCort iQ is not an ArcGIS or Google Maps replacement."
    ),
    "INT-027 — Microsoft Teams: NexCort iQ has an active Microsoft Teams integration built on the Microsoft Graph API. The integration supports:": (
        "INT-027 — Microsoft Teams: In-platform NexCort iQ war rooms are available for incident command. Deep Microsoft Teams meeting and channel integration is an optional Year-1 module ($8,000/year) using Microsoft Graph APIs, subject to IU licensing and admin consent. Planned capabilities:"
    ),
    "War room creation in RC automatically generates a Teams meeting — participants receive a join link directly in the RC war room interface": (
        "Optional: war room creation can generate a Teams meeting join link in the NexCort iQ war room interface"
    ),
    ...
    "Describe the incident in their own language — RC automatically translates reports from 40-plus languages into English for the security console.": (
        "Describe the incident in their own language — NexCort iQ translates written reports across 100-plus language codes into English for the security console (speech/STT uses a configured subset)."
    ),
    ...
    "Computer vision: Live camera feed analysis for object detection and activity classification via connected VMS analytics platforms.": (
        "Computer vision: object detection and activity classification from IU’s VMS/camera analytics (ingested events), not a NexCort iQ-native CV engine."
    ),
    ...
    "Records retention configurable to meet Clery Act's seven-year minimum requirement for all alert and incident records.": (
        "Records retention configurable to IU policy, including a seven-year setting to support Clery-adjacent incident records. NexCort iQ does not file the Annual Security Report."
    ),
}
...
TABLE_CELL_REPLACEMENTS = [
    ...
    ("AWS infrastructure: SOC 2 Type II, ISO 27001, FedRAMP (Moderate). RC platform: SOC 2 in progress (Q2 2027).", "AWS inherited controls (SOC 2 / ISO / FedRAMP on AWS services). NexCort iQ application: SOC 2 Type II not completed; HECVAT and alignment documentation will be provided."),
...
def align_docx() -> None:
                ...
                for cell in row.cells:
                    if "FedRAMP" in cell.text:
                        replace_cell_text(
                            cell,
                            cell.text,
                            "AWS inherited controls (SOC 2 / ISO / FedRAMP on AWS services). NexCort iQ application: no SOC 2 Type II report claimed; HECVAT and alignment documentation will be provided.",
                        )
    ...
    leftovers = {
        ...
        "40-plus": "100-plus",
        "RapidIQ AI": "NexCort iQ incident intelligence",
        "RapidIQ": "NexCort iQ incident intelligence",
