...
## 5. Media — citizen & facility cameras (`/<slug>/media`)

When **Nest Connect** or **Wyze Connect** is enabled for your agency:

1. **Nest** / **Wyze** tabs appear under **Live Camera** (next to **Facility**).
2. **View nearby cameras** — lists cameras near the selected incident (requires active incident + GPS on devices).
3. **Send Emergency Video Request** — per device; owner must **approve** via SMS/email link before any live video.

Train floor staff: approval is **voluntary** and **time-limited**.
