...
**Companion:** [SUPPORT_MODEL.md](./SUPPORT_MODEL.md) (first responder matrix). This page orders **when** to move from agency desk → IT → NexCort iQ → engineering. **Internal operational playbook:** [ADMIN_TEAM_TROUBLESHOOTING_GUIDE.md](./ADMIN_TEAM_TROUBLESHOOTING_GUIDE.md).
...
3. If API returns persistent `FORBIDDEN` despite correct attributes — **NexCort iQ pilot** with HAR + `requestId`.
...
3. **NexCort iQ** checks Lambda logs / quotas ([RUNBOOK.md](./RUNBOOK.md) “AI analyze failures”).

### Multilingual / translation / STT

1. Confirm **Admin → Configuration / Integrations** issue count.
2. **NexCort iQ** secrets + [`RUNBOOK_MULTILINGUAL_CALLS.md`](./RUNBOOK_MULTILINGUAL_CALLS.md).

### Vendor / provider outages (OpenAI, Bedrock, Azure, Google)

1. Check vendor status pages.
2. **NexCort iQ** may switch tiers or declare degraded mode per runbook — **not** agency IT alone.
