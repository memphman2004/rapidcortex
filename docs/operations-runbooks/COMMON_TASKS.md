...
### Connect — request citizen camera video (per device)

1. On **`/<slug>/media`**, select an **active incident** in **Incident Context**.
2. Select the **Nest** or **Wyze** tab and list nearby cameras.
3. Pick a device card (status **Available**).
4. Choose duration → **Send Emergency Video Request** → confirm.
