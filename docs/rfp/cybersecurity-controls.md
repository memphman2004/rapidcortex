...
**Scope:** NexCort iQ cloud SaaS (web + API on AWS). Assistive intelligence for emergency communications — does **not** replace CAD, 911 telephony, radio, or medical direction.  
...
NexCort iQ is designed so an agency CJIS Information Security Officer can **map** product controls to the CJIS Security Policy. The agency completes its own authorization path (personnel, physical, and local policy). NexCort iQ does not issue FBI approval.

| CJIS-oriented area | NexCort iQ control | Evidence |
|--------------------|----------------------|----------|
...
**Agency still owns:** background checks, security awareness training, physical PSAP controls, CJIS Security Addendum execution, and mapping of NexCort iQ as a contractor system.
...
NexCort iQ provides **platform security operations** on AWS — not a customer-owned SOC floor and not a default 24/7 human watch center.
...
|------|----------|--------------------|
| **Standard (included)** | CloudWatch + SNS; business-hours NexCort iQ engineering on-call per [SUPPORT_MODEL.md](../operations-runbooks/SUPPORT_MODEL.md) | Default |
| **24/7 incident support** | Dedicated on-call NexCort iQ engineer under contracted SLA | Add-on `reliability.support_upgrade_247` |
...
NexCort iQ is multi-tenant **SaaS**. Segmentation is applied in AWS and in the data model — not by installing VLANs in the PSAP.
...
| Agency LAN | **Agency IT.** NexCort iQ users reach the SaaS over HTTPS; CAD/radio stay on agency networks |

**Agency responsibility:** segment CAD, logging recorders, and 911 CPE from general office VLANs per local policy. NexCort iQ does not require inbound connections from the cloud into the CAD VLAN for the default assistive (read-side / no write-back) posture.

Draw the **agency-specific** access path (workstations → NexCort iQ URL, optional CAD interface) in the implementation workbook. Do not reuse a generic diagram as if it were that site’s network.
...
| Privileged platform access | `rcsuperadmin` / `rcadmin` / `rcitadmin` only on NC Admin; cross-tenant access logged |
...
Access reviews: agency IT reviews active Cognito users at least **quarterly**; NexCort iQ reviews platform-operator accounts on the same cadence.
...
5. **Patch** — Critical/high dependency findings: target **14 days**; medium **30 days**; or documented risk acceptance by NexCort iQ security owner.  
...
8. **Disclosure** — Security issues to NexCort iQ security contact; no public zero-day detail until patched.
...
An **incident** here is an operational or security event affecting NexCort iQ (outage, suspected breach, auth anomaly) — not a 911 call record.
...
Tabletop: NexCort iQ runs an IR tabletop at least **annually** and after material architecture change; record date in the agency playbook if they participate.
...
Restore drill steps: [BACKUP_AND_RECOVERY.md](../operations-runbooks/BACKUP_AND_RECOVERY.md). Floor continuity if NexCort iQ is down: dispatchers use CAD/radio/911 as today ([NON_GOALS.md](../go-to-market-sales/NON_GOALS.md)).
