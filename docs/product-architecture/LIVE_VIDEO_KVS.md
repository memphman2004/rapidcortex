...
- **SMS**: `SMS_PROVIDER=aws` (or `mock` in lower environments). SMS is **link delivery only**. Outbound identity (**`+14707482763`** on AWS End User Messaging) is **send-only**; `callerPhone` on **`POST .../live-video/request`** must always be the **caller's mobile** (E.164), not the sending number.
