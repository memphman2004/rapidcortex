# System boundary — NexCort iQ (SOC 2)
...
This is the in-repo system-boundary description. A signed copy may also live in the private compliance share (`Business Documents/Compliance/NexCort iQ Compliance/01 - Governance/SYSTEM-BOUNDARY.md`). If the two diverge, **update this file** so git matches what auditors are told.
...
| Legal entity | Apps on Demand LLC d/b/a NexCort iQ (entity naming still open — LEG-007) |
...
- AWS: Lambda, API Gateway HTTP APIs, DynamoDB, S3 (NexCort iQ buckets), Cognito, CloudWatch, SNS/SES (transactional), Secrets Manager, ACM, CloudFront, WAF (CloudFront-scope ACLs), ECS/Fargate (web SSR), Route 53 records for NexCort iQ hosts, WAF logging groups named `aws-waf-logs-rapid-cortex-*`.
...
## 2. Explicitly out of scope (NexCort iQ SOC 2)
...
| Other products in account `158961537080` (carve-out below) | Not NexCort iQ |
...
Account `158961537080` also hosts **non–NexCort iQ** workloads historically referred to as Melios, MindHeist, PassPoint, OR Game, and personal/marketing sites.

**SOC 2 treatment (chosen control):** NexCort iQ production remains in this account for the October 2026 observation window. Other products are **out of NexCort iQ system scope** under all of the following compensating controls:

1. **Naming:** NexCort iQ resources use `rapid-cortex-*` / `RapidCortex*` names. PITR, S3 encryption/BPA, and CloudTrail data-event selectors are applied to those names.
2. **IAM:** Production deploy principal `rapid-cortex-deploy` and auditor role `rapid-cortex-soc2-auditor` are NexCort iQ–named. Human access to NexCort iQ data is reviewed quarterly ([processes/access-review.md](./processes/access-review.md)).
3. **Data:** Customer incident/transcript data lives in NexCort iQ DynamoDB tables and NexCort iQ S3 buckets only.
4. **Logging:** In-scope CloudTrail is `rapid-cortex-cloudtrail-prod` with S3 data events on `rapid-cortex-*` buckets.
5. **AccessDenied on other-product buckets** in the 2026-09-17 snapshot is expected and is **not** treated as a NexCort iQ control failure.
...
- Reviewing NexCort iQ audit events for their `agencyId`.
...
|---------|----------------------------------|--------------------|---------|
| DynamoDB PITR | 182/182 NexCort iQ/Ring **ENABLED** | `DynamoPointInTimeRecovery=auto` + `DeploymentStage=dev` would turn **AppSam** PITR off (data-layer already treats `dev` as pilot-grade) | Force `DDB_ENABLE_PITR=true`; add `dev` to `EnablePilotGradeBackups` on AppSam nests |
