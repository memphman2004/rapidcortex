...
export async function generateMetadata(): Promise<Metadata> {
  return {
    ...
    openGraph: {
      ...
      images: [buildOgShareImage("Rapid Cortex product overview")],
      type: "website",
    },
    twitter: {
    ...
    <article className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="space-y-4">
        ...
        <p className="max-w-3xl text-sm text-slate-300">
          ...
          non-emergency / 311 Call Assist, campus safety, and venue command environments.
        </p>
      </header>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <section className="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
          <h2 className="text-lg font-medium text-white">Call Assist</h2>
          <p className="mt-2 text-sm text-slate-300">
            Non-emergency / 311 AI intake so routine volume never sits in the 911 chair. Escalates to a live
            call taker when urgency appears.
          </p>
          <Link href="/#non-emergency-ai" className="mt-4 inline-block text-sm font-medium text-sky-300 hover:text-sky-200">
            View non-emergency AI
          </Link>
        </section>
        <section className="rounded-lg border border-slate-800 bg-slate-900/40 p-4">
