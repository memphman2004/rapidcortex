...
function severityDot(severity: VisionSceneAlert["severity"]): string {
...
async function fetchAlerts(status: FilterTab): Promise<VisionSceneAlert[]> {
  ...
  if (!res.ok) return [];
  const body = (await res.json()) as { success?: boolean; data?: { alerts?: VisionSceneAlert[] } };
...
export function CameraAiAlertsPanel() {
  ...
  const handleCreated = async (result: CreateIncidentResult) => {
    ...
    await patchAlert(draftAlert.eventId, "create_incident", result.incidentId);
    setDraftOpen(false);
    void queryClient.invalidateQueries({ queryKey: ["vision-scene-alerts"] });
  };

  const handleDismiss = async (alert: VisionSceneAlert) => {
    await patchAlert(alert.eventId, "dismiss");
    void queryClient.invalidateQueries({ queryKey: ["vision-scene-alerts"] });
  };
          ...
          <ul>
            {alerts.map((alert) => (
              <li key={alert.eventId}>
                <button
                  type="button"
                  onClick={() => setSelectedId(alert.eventId)}
                ...
                >
                  ...
                  />
                  <span className="w-12 shrink-0 font-mono text-[11px]" style={{ color: V.muted }}>
          ...
          <article className="m-2 rounded-lg border p-3" style={{ borderColor: V.border, background: V.surface }}>
            <div className="mb-2 flex items-center gap-2 text-[11px] font-bold tracking-widest">
              <span
                ...
                style={{ background: severityDot(selected.severity) }}
              />
              CAMERA ALERT
            ...
            {selected.narrative ? (
              <p className="mb-3 text-[12px] leading-relaxed" style={{ color: V.text }}>
                “{selected.narrative}”
              </p>
            ) : (
            ...
            <div className="flex flex-wrap gap-1.5">
              <a
                href={to(`/media?vision=1&camera=${encodeURIComponent(selected.cameraId)}`)}
