...
async function proxyWebhook(request: NextRequest, agencyId: string, slot: string) {
  const upstreamPath = `/api/public/c2c/${encodeURIComponent(agencyId)}/${encodeURIComponent(slot)}/events`;
  const base = resolveUpstreamApiBase(upstreamPath);
  if (!base) {
    return NextResponse.json({ error: "API_UPSTREAM_BASE_2 is not configured for C2C public routes" }, { status: 503 });
  }
