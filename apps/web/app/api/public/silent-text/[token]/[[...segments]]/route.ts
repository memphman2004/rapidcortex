...
export async function GET(request: NextRequest, ctx: Ctx) {
  ...
  if (!segments?.length) {
    return proxyUpstream(`/api/silent-text/t/${enc}`);
  }
  if (segments.length === 1 && segments[0] === "messages") {
    return proxyUpstream(`/api/silent-text/t/${enc}/messages`);
  }
...
export async function POST(request: NextRequest, ctx: Ctx) {
  ...
  const map: Record<string, string> = {
    opened: `/api/silent-text/t/${enc}/opened`,
    presence: `/api/silent-text/t/${enc}/presence`,
    message: `/api/silent-text/t/${enc}/message`,
    end: `/api/silent-text/t/${enc}/end`,
  };
  const path = map[sub];
