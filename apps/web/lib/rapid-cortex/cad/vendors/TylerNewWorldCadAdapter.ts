...
export class TylerNewWorldCadAdapter implements CadAdapter {
  ...
  async healthCheck(): Promise<CadHealthResult> {
    try {
      ...
      return {
        ok: true,
        mode: "read_only",
        provider: "tyler-new-world",
        detail: "Tyler New World CAD read-only adapter healthy.",
      };
    } catch (error) {
      return {
        ok: false,
        mode: "read_only",
        provider: "tyler-new-world",
        detail: error instanceof Error ? error.message : "Tyler health check failed.",
      };
    }
  }
  ...
  async searchIncidents(query: CadSearchQuery): Promise<CadIncident[]> {
    ...
    return list.map((item, index) => this.mapIncident(item, `tyler-incident-${index}`));
  }
  ...
  private ensureConfigured(): void {
    if (!this.baseUrl) throw new Error("CAD_API_BASE_URL is required for Tyler New World CAD adapter.");
    if (!this.apiKey) throw new Error("CAD_API_KEY is required for Tyler New World CAD adapter.");
  }
  ...
  private mapIncident(raw: unknown, fallbackId: string): CadIncident {
    ...
    return {
      incidentId: String(
        firstString(rec, ["eventNumber", "EventNumber", "callNumber", "call_number", "id"]) ?? fallbackId,
      ),
      status: firstString(rec, ["eventStatus", "dispatch_status", "status"]),
      callType: firstString(rec, ["callType", "call_type", "CallType", "nature"]),
      location: firstString(rec, ["locationAddress", "location_text", "address", "location"]),
      units: asUnitList(rec.assignedUnits ?? rec.apparatus ?? rec.units),
      lastUpdatedAt: firstString(rec, ["updatedAt", "lastUpdatedAt"]),
      raw: rec,
    };
  }
}
