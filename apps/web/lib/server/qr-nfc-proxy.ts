...
/**
 * BFF proxy for QR/NFC. `/global` is a dedicated list route — never treat it as a qrId.
 */
export async function proxyQrNfc(request: NextRequest, segments: string[]): Promise<NextResponse> {
  ...
  const isEngage = segments[segments.length - 1] === "engage";
