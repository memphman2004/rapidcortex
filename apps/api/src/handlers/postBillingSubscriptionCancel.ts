...
import { cancelSubscriptionBodySchema } from "rapid-cortex-shared";
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const parsed = cancelSubscriptionBodySchema.safeParse(JSON.parse(event.body ?? "{}"));
    if (!parsed.success) return badRequestFromZod(parsed.error);
    const profile = await service.cancelAtPeriodEnd(user, agencyId, parsed.data.reason);
    return ok(profile);
  } catch (error) {
