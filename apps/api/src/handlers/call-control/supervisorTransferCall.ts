...
import { transferCallBodySchema } from "rapid-cortex-shared";
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const parsed = transferCallBodySchema.safeParse(JSON.parse(event.body ?? "{}"));
    if (!parsed.success) return badRequestFromZod(parsed.error);

    const result = await service.transferCall(user, parsed.data);
    return ok(result);
  } catch (e) {
