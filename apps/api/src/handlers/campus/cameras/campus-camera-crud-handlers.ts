...
import {
  buildCampusProducerConfigYaml,
  createCampusCamera,
  deleteCampusCamera,
  discoverCampusCamera,
  listCampusCameras,
  recordCampusProducerAgentHeartbeat,
  updateCampusCamera,
...
function requireSupervisorOrAgencyIt(
  ...
  if (!canSupervisorCampusOps(user as never)) {
...
function producerKeyAuthorized(event: Parameters<APIGatewayProxyHandlerV2>[0]): boolean {
  const expected = process.env.CAMPUS_CAMERA_PRODUCER_KEY?.trim() || process.env.VENUE_CAMERA_PRODUCER_KEY?.trim();
...
export const listOrCreate: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    if (event.requestContext.http.method === "GET") {
      const cameras = await listCampusCameras(ctx.agencyId);
    ...
    const camera = await createCampusCamera(ctx.agencyId, ctx.user.userId, parsed.data);
    return withCorrelationHeaders(event, ok({ camera }));
  } catch (error) {
    console.error("[campus-camera-registry-listOrCreate]", error);
...
export const updateOrDelete: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    if (event.requestContext.http.method === "DELETE") {
      await deleteCampusCamera(ctx.agencyId, cameraId, ctx.user.userId);
    ...
    const camera = await updateCampusCamera(ctx.agencyId, cameraId, ctx.user.userId, parsed.data);
    return withCorrelationHeaders(event, ok({ camera }));
  } catch (error) {
    ...
    console.error("[campus-camera-registry-updateOrDelete]", error);
    return withCorrelationHeaders(event, serverError());
  }
};
...
export const viewerToken: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    console.error("[campus-camera-viewer-token]", error);
    return withCorrelationHeaders(event, serverError());
  }
};
...
export const ptz: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    console.error("[campus-camera-ptz]", error);
    return withCorrelationHeaders(event, serverError());
  }
};
...
export const producerConfig: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const cameras = await listCampusCameras(ctx.agencyId);
    const yaml = buildCampusProducerConfigYaml(ctx.agencyId, cameras);
    const filename = `rc-kvs-producer-campus-${ctx.agencyId.replace(/[^a-zA-Z0-9-]/g, "-")}.yaml`;
    ...
    console.error("[campus-camera-producer-config]", error);
    return withCorrelationHeaders(event, serverError());
  }
};
...
export const discover: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const discovered = await discoverCampusCamera(parsed.data);
    ...
    console.error("[campus-camera-discover]", error);
    return withCorrelationHeaders(
      event,
...
export const agentHeartbeat: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const camera = await recordCampusProducerAgentHeartbeat(agencyId, cameraId);
    ...
    console.error("[campus-camera-agent-heartbeat]", error);
    return withCorrelationHeaders(event, serverError());
  }
};
