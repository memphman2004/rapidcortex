...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const parsed = createPremiseNoteRequestSchema.safeParse(JSON.parse(event.body ?? "{}"));
    ...
    let out;
    try {
      out = await service.createPremiseNote(incidentId, user, parsed.data);
    } catch (e: unknown) {
      if (e instanceof Error && e.message === "NO_ADDRESS_ON_INCIDENT") {
        return badRequest("Set a caller address on the incident before adding premise notes.");
      }
      throw e;
    }
    if (out === null) return forbidden("Premise notes unavailable or access denied");
    ...
    await auditRepo.create({
      ...
      incidentId,
      actorId: user.userId,
      type: AUDIT_EVENT_TYPES.PREMISE_NOTE_CREATED,
