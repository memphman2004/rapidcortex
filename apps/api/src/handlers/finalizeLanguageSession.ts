...
import { finalizeLanguageSessionBodySchema } from "rapid-cortex-shared";
import {
  badRequest,
  forbidden,
  jsonStatus,
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const parsed = finalizeLanguageSessionBodySchema.safeParse(JSON.parse(event.body ?? "{}"));
    ...
    await service.finalizeSession(incidentId, user, parsed.data.sessionId);
    return ok({ ok: true });
  } catch (error) {
