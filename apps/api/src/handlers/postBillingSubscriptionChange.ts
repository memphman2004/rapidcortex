...
import { changeSubscriptionPlanBodySchema } from "rapid-cortex-shared";
...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  try {
    ...
    const parsed = changeSubscriptionPlanBodySchema.safeParse(JSON.parse(event.body ?? "{}"));
    if (!parsed.success) return badRequestFromZod(parsed.error);
    const profile = await service.changePlan(user, agencyId, parsed.data);
    return ok(profile);
  } catch (error) {
