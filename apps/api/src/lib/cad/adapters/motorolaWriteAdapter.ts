...
export const motorolaWriteAdapter: CadWriteAdapter = {
  ...
  async submit({ payload, config, cadIncidentId }) {
    ...
    if (!apiUrl) {
      return {
        success: true,
        cadResponse: JSON.stringify({ mock: true, vendor: "motorola_premier_one", reason: "apiUrl not set" }),
      };
    }
    const apiKey = (config.apiKey ?? "").trim();
    const url = `${apiUrl.replace(/\/$/, "")}/api/incidents/${encodeURIComponent(cadIncidentId)}/narrative`;
    const body = {
      NarrativeText: payload.narrative,
      Priority: payload.priority,
      Units: payload.units,
      NatureCode: payload.cadNatureCode,
    };
    try {
