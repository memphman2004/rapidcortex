 ...
 *
 * NexCort iQ enhances operations. It does not replace CAD, 911, radio, ENS, or VMS.
 */
  ...
  if (input.vertical === "CAMPUS") {
    return [
      ...
      },
      {
        ...
        body: `Campus ENS products blast alerts. NexCort iQ is the intake and coordination layer for what students actually report: location-aware incidents, two-way text with security when you enable it, and structured records that help Clery documentation.\n\nIT usually asks about FERPA and logs: we design for no PII in operational CloudWatch logs and anonymous reporting options. Your counsel still owns policy.\n\nHappy to demo the student path or send the campus one-pager.`,
      },
      {
  ...
  if (input.vertical === "VENUE") {
    return [
      ...
      },
      {
        ...
        body: `We do not ask you to rip out the camera platform. NexCort iQ registers venue cameras you already have (RTSP / ONVIF) so a section incident can open the relevant views in the same console as the guest report.\n\nLegal and risk usually want consent and retention in writing before a public QR program. We treat that as a joint plan, not a checkbox in a slide.\n\nHappy to walk the console or send the venue overview.`,
      },
      {
  ...
  return [
    {
      ...
      body: `${signal}Dispatchers at ${org} still have to listen, type, and decide on the same call. NexCort iQ is a browser co-pilot for 911 / ECC staff: live transcription, AI-assisted incident structure, and supervisor visibility. It does not replace CAD, CPE, radio, or the dispatcher.\n\nWould a 20-minute walkthrough this week be useful?`,
    },
    {
      subject: `Re: ${org} — CAD stays the system of record`,
      body: `CAD remains the system of record. NexCort iQ is an intelligence layer: transcripts, suggested structure, protocol-aligned coaching when your agency has approved packs, and QA surfaces for supervisors.\n\nCAD write-back is off unless you later sign a scoped connector project. We start most agencies on a standalone pilot so IT is not blocked on a vendor program.\n\nHappy to send a one-page technical overview or walk the dispatcher workspace live.`,
    },
    {
      subject: `${org} — what supervisors actually see`,
      body: `Last note unless you want a working session. Supervisors use NexCort iQ for a second look: searchable transcripts, flags for review, and coaching that follows agency-approved protocol packs. Translation runs when your deployment wires that pipeline — it does not replace interpreters.\n\nIf the timing is wrong, say so. If a 20-minute floor walkthrough is useful, I will keep it tight.`,
    },
  ];
}
