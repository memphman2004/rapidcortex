...
function tableName(): string {
  const t = process.env.VENUE_CAMERA_REGISTRY_TABLE?.trim();
  if (!t) throw new Error("VENUE_CAMERA_REGISTRY_TABLE not set");
  return t;
}

export class VenueCameraRegistryRepository {
  ...
  /** Heartbeat job — scan all registry rows (bounded by venue camera count). */
